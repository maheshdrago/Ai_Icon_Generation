...
const Dashboard = ({ is_loggedin }: { is_loggedin: boolean }) => {
  ...
  const generateImage = async () => {
    const response = await axiosInstance.post("/image/generate", {
      prompt:"A abstract cat with goggles in eyes"
    })
  ...
  return (
    <div className="flex flex-col items-center justify-center  w-full">
      ...
      <div className="flex flex-col justify-start items-start w-1/4 gap-2">
        ...
        <input placeholder="an angry bird" className="w-full p-2"></input>
      </div>
      ...
      <SliderTab
        ...
        activeTab={activeTab}
        className="flex flex-col justify-center items-center gap-4 w-2/6"
      >
        ...
        <SliderTabItem className=""></SliderTabItem>
      </SliderTab>
        ...
        <div className="icon-styles ">
          {IconStyles.map((icon) => (
            <div
              ...
              className={`relative h-24 w-24 rounded-md shadow-md cursor-pointer hover:scale-[1.2] transition-all delay-100 ease-in-out ${icon.toLowerCase() === iconStyle ? "scale-110": ""}`}
              onClick={() => setIconStyle(icon.toLowerCase())}
            >
              <div
                className={`absolute inset-0  opacity-50 rounded-md ${icon.toLowerCase() === iconStyle ? "block": "hidden"}`}
                style={{
                  backgroundColor: "rgba(0, 0, 0, 0.3)", 
                }}
              ></div>
              <div className={`absolute top-1 right-1 bg-white rounded-full p-1 ${icon.toLowerCase() === iconStyle ? "block": "hidden"}`}>
                <svg
